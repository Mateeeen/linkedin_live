      ...
      if (userData.length > 0) {
        ...
        userData.map((item) => {
          document.querySelector(".groupBoxContainer").innerHTML += `
                ...
                <br> <span style="color:red">Total prospects: </span>${item.prospects}
                </span>
        ...
        document.querySelectorAll(".groupBox").forEach((ele) => {
          ele.addEventListener("hover", hovername);
        });
      } else {
        document.querySelector(".groupBoxContainer").innerHTML = "";
                ...
                <div class="prospectContentContainer">
                  <div class="prospectContent prospectChatContent1" data-prospect_id=${
                    ...
                    <div>
                      ...
                      <h2>${
                        ele.sender_name != null ? `By ${ele.sender_name} ${ele.database != null ? `from ${ele.database}` : ''}` : ""
                      }</h2>
                    </div>
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        if (userData.status == "Updated") {
          ...
          document.getElementById("groupNamePara").innerHTML =
        `<span style='color: #084DD1;'>Group Name: </span>${userData.name.length > 10 ? `${userData.name.slice()} ...` : userData.name}`;
...
function groupDeleteion(e) {
  ...
  if(localStorage.getItem("group_id")){
    ...
    group_id = e.currentTarget.parentElement.querySelector('.groupBox').getAttribute('data-group_id');
  }
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          document
            ...
            document.getElementById("groupNamePara").innerHTML =
            `<span style='color: #084DD1;'>Group Name: </span>${userMessages.group_name.length > 10 ? `${userMessages.group_name.slice()} ...` : userMessages.group_name}`;
...
if(document.getElementById('showMembers')){
  document.getElementById('showMembers').addEventListener('click', dynamicModalOpenM)
}
...
function showProspectChat(e) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      document.getElementById("companyName").innerText =
        `(${userData.prospect_data.company})`;
        }
      document.getElementById("descriptionName").innerText =
        userData.prospect_data.description.slice(0, 45) +'...'
        document.getElementById("groupNamePara").innerHTML =
        `<span style='color: #084DD1;'>Group Name: </span>${userData.group_name.length > 10 ? `${userData.group_name.slice()} ...` : userData.group_name}`;
      document.getElementById('dbNameMessage').innerText = 
      `${
...
if (document.getElementById("showDirectBox")) {
  document.getElementById("showDirectBox").addEventListener("click", showNullChats);
}
...
function showNullChats() {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      document.getElementById('dbNameMessage').innerText = 
      `${
        userData.sender_name != null ? `Clipper: ${userData.sender_name} ${userData.database != null ? ` Database: ${userData.database}` : ''}` : ""
      }`
